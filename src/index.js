...
const move = (index) => {
  ...
  if (victory) {
    ...
    audioWin.play();
    setTimeout(() => {
      field.innerHTML = `<div class="victory">Hooray!<br><br>You solved<br>the puzzle<br>in ${time.innerHTML.replace(
        'Time: ',
        ''
      )}<br>and ${counter} moves</div>`;
    }, 500);
    clearInterval(interval);
    if (!results) {
      results = [];
      ...
      results.sort(
        (a, b) =>
          b.resultsFieldSize[0] - a.resultsFieldSize[0] ||
          a.moves - b.moves ||
          +`${a.time[0]}${a.time[1]}` * 60 +
            +`${a.time[3]}${a.time[4]}` -
            (+`${b.time[0]}${b.time[1]}` * 60 + +`${b.time[3]}${b.time[4]}`)
      );
    }
    ...
    for (let i = 0; i < results.length; i++) {
      if (i < 10) {
        resultsItemsArray[i].innerHTML = `${i + 1}. ${results[i].resultsFieldSize} - ${
          results[i].moves
        } moves - time: ${results[i].time}`;
      }
    }
  }
};
